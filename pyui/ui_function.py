...
from widgets.Combo_box import ComboBox_Equation
...
class UIFunction(MainWindow):
    ...
    def toodleMenu(self, maxWidth, clicked):
        ...
        if clicked:
            ...
            self.animation = QtCore.QPropertyAnimation(self.ui.frame_bottom_west, b"minimumWidth")
            ...
            if self.ui.frame_bottom_west.width()==80:
                ...
                if (len(self.equations)>1):
                    ...
                    for i in range(0, len(self.equations)):
                        self.ui.gridLayout.removeItem(self.ui.gridLayout.itemAtPosition(0, i))
                        
                    print(len(self.equations))
                    for item in range(0, len(self.equations)):
                        self.equations[item].layout().deleteLater()
                        
                    self.equations.clear()
                    self.sizeE = 0
                    
                    UIFunction.Equations(self)
                    """
                #else:
                    #UIFunction.Equations(self)
    ...
    def Equations(self):
        ...
        def Equation():
            ...
            if(len(self.equations) <= 5):
                self.equations.append(QVBoxLayout())
                self.equations[self.sizeE].addLayout(QHBoxLayout())
                size = self.sizeE
                
                self.equations[self.sizeE].itemAt(0).layout().addWidget(ComboBox_Equation())
                if(len(self.equations) <= 5):
                    self.equations[self.sizeE].itemAt(0).layout().addWidget(PushButton(ico=QIcon(os.path.abspath("images/plus.png"))))
                    #self.equations[self.sizeE].itemAt(0).layout().addItem(QSpacerItem(30, 20, QSizePolicy.Minimum, QSizePolicy.Expanding))
                    self.equations[self.sizeE].itemAt(0).layout().itemAt(1).widget().clicked.connect(Equation)
                    self.equations[self.sizeE].itemAt(0).layout().itemAt(1).widget().setFixedSize(20, 20)
                self.equations[self.sizeE].itemAt(0).layout().itemAt(0).widget().activated.connect(lambda : on_combobox_item_selected(self.ui, self.equations[size].itemAt(0).layout().itemAt(0).widget().currentIndex(), 
                                                                                                                                        size, self.document))
                self.ui.gridLayout.addLayout(self.equations[self.sizeE], 0, size)
                self.sizeE+=1 
            
        def on_combobox_item_selected(ui, index, s, document):
            self.equations[s].itemAt(0).layout().itemAt(0).widget().actions[index](ui, s, document, self.word )
            self.ui.doc_but.setDisabled(False)

        Equation()

    #def actions(box):
      #      self.act = [self.on_action_triggered(), self.on_action2_triggered(), self.on_action3_triggered(), self.on_action4_triggered(),
      #         self.on_action5_triggered()]
     #   
      #      for i in range(5):
      #          box.actions_equa[i].triggered.connect(self.act[i])
